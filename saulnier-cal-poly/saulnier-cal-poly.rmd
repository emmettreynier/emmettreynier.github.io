...
3. Simulate .hi-pink[counterfactual climate] scenarios  
  ...
  - To-date, climate damages are over **2x larger** for black vs white HH's  
...
.pull-left[
#### .bold[ [Introduction](#intro) ]
#### [<span style="color: #A8A8AA">**Model**</span>](#model) 
#### [<span style="color: #A8A8AA">**Data**</span>](#data)
#### [<span style="color: #A8A8AA">**Estimation**</span>](#estimation)
#### [<span style="color: #A8A8AA">**Simulations**</span>](#sim-intro)
#### [<span style="color: #A8A8AA">**Conclusion**</span>](#next-steps)
]

.pull-right[
<center>Change in .hi-blue[cold] days</center>

```{r, out.width = "80%", fig.align='center'}

knitr::include_graphics(
  path = "figures/climate/prism-grid-cold-days-1990-2019.jpeg"
)

```

<center>Change in .hi-red[hot] days</center>
...
.pull-left[
#### [<span style="color: #A8A8AA">**Introduction**</span>](#intro) 
#### .bold[ [Model](#model) ]
#### [<span style="color: #A8A8AA">**Data**</span>](#data)
#### [<span style="color: #A8A8AA">**Estimation**</span>](#estimation)
#### [<span style="color: #A8A8AA">**Simulations**</span>](#sim-intro)
...
### Consider Jacksonville, FL as an example  

--

There are now more hot days than there were in the 80's

```{r, fig.dim = c(6*1.618, 6), out.width = "80%"}

prism_cbsa_daily_dt = 
  read.fst(
    path = 'data/prism-cbsa-daily-dt.fst',
    as.data.table = TRUE
  )[,
    period := fcase(
      time < ymd('1991-01-01'), '1990',
      time >= ymd('2015-01-01'), '2019'
    )
  ]
jacksonville_temp_distr = 
  ggplot(
    prism_cbsa_daily_dt[cbsa_fip == '27260' & !is.na(period) & variable == 'tmean'], 
    aes(x = (value*(9/5) + 32), fill = period)
  ) + 
  ...
  scale_fill_brewer(palette = 'Dark2', name = 'Period (5 year avg)') + 
  scale_y_continuous(
    name = "Number of Days Annually", 
    breaks = seq(0, 80, by = 20)/(365*5),
...
- **Amenities decrease** relative to other cities    
- **More costly to produce comfort** as HH's use A/C more intensely     

--

$\implies$ conditional on living in Jacksonville, utility has decreased.  
...
### Mechanisms that could cause .pink[unequal distribution]  
...
--
1. **Comfort costs are higher**, may be more sensitive to climate     
...
$$\log V_{ij} =\tilde{\alpha}_{d}\log(W_{dj}) - \alpha_d^\mathcal{C} \log(P_{dj}^\mathcal{C}) + \alpha^Z_d \cdot Z_{j} + g(j,\boldsymbol{b}_i) +\xi_{dj}+ \sigma_d\varepsilon_{ij}$$
...
$$\log V_{ij} =\color{#EC7662}{\tilde{\alpha}_{d}\log(W_{dj})} - \alpha_d^\mathcal{C} \log(P_{dj}^\mathcal{C}) + \alpha^Z_d \cdot Z_{j} + g(j,\boldsymbol{b}_i) +\xi_{dj}+ \sigma_d\varepsilon_{ij}$$

- $\color{#EC7662}{W_{dj}}$ are wages for demographic group $d$ in city $j$  
...
$$\log V_{ij} =\tilde{\alpha}_{d}\log(W_{dj}) - \color{#EC7662}{\alpha_d^\mathcal{C} \log(P_{dj}^\mathcal{C})} + \alpha^Z_d \cdot Z_{j} + g(j,\boldsymbol{b}_i) +\xi_{dj}+ \sigma_d\varepsilon_{ij}$$
...
$$\log V_{ij} =\tilde{\alpha}_{d}\log(W_{dj}) - \alpha_d^\mathcal{C} \log(P_{dj}^\mathcal{C}) + \color{#EC7662}{\alpha^Z_d \cdot Z_{j}} + g(j,\boldsymbol{b}_i) +\xi_{dj}+ \sigma_d\varepsilon_{ij}$$
...
$$\log V_{ij} =\tilde{\alpha}_{d}\log(W_{dj}) - \alpha_d^\mathcal{C} \log(P_{dj}^\mathcal{C}) + \alpha^Z_d \cdot Z_{j} + \color{#EC7662}{g(j,\boldsymbol{b}_i)} +\xi_{dj}+ \sigma_d\varepsilon_{ij}$$
...
$$\log V_{ij} =\tilde{\alpha}_{d}\log(W_{dj}) - \alpha_d^\mathcal{C} \log(P_{dj}^\mathcal{C}) + \alpha^Z_d \cdot Z_{j} + g(j,\boldsymbol{b}_i) +\color{#EC7662}{\xi_{dj}}+ \sigma_d\varepsilon_{ij}$$
...
$$\log V_{ij} =\tilde{\alpha}_{d}\log(W_{dj}) - \alpha_d^\mathcal{C} \log(P_{dj}^\mathcal{C}) + \alpha^Z_d \cdot Z_{j} + g(j,\boldsymbol{b}_i) + \xi_{dj}+ \color{#EC7662}{\sigma_d\varepsilon_{ij}}$$
...
$$\log V_{ij} =\tilde{\alpha}_{d}\log(W_{dj}) - \color{#EC7662}{\alpha_d^\mathcal{C} \log(P_{dj}^\mathcal{C})} + \color{#EC7662}{\alpha^Z_d \cdot Z_{j}} + g(j,\boldsymbol{b}_i) + \xi_{dj}+ \sigma_d\varepsilon_{ij}$$
...
2. **Climate Data**: Daily temperature and precipitation from PRISM and natural disaster risk from the First Street Foundation    
...
We'll focus on the parameters of the utility function. 

.footnote[[[labor demand details]](#labor-demand-est-details) [[housing supply details]](#housing-supply-est-details)]
...
$$\color{#EC7662}{\delta_{djt}} = \beta_{e}^w\log(W_{djt}) + \beta_e^\mathcal{C}  \log(P_{djt}^c) + \beta^Z_e \cdot Z_{jt}+ \xi_{djt}$$
...
$$\delta_{djt} = \beta_{e}^w\log(W_{djt}) + \beta_e^\mathcal{C}  \log(P_{djt}^c) + \beta^Z_e \cdot Z_{jt}+ \xi_{djt}$$
...
which we maximize by making guesses of $\boldsymbol{\gamma_{dt}}$ using the Nelder-Mead algorithm, recovering the $\delta_{djt}$'s using BLP contraction mapping. 
...
--
1. Calibrate coefficients on $\small\log{W_{djt}}$ using **Diamond (2016)**
2. Calibrate coefficients on $\small\log(P_{djt}^c)$ using **comfort expenditure shares**   
3. Calculate residual, $\small\hat{\delta}_{djt} = \delta_{djt} - \beta_{e}^w\log(W_{djt}) + \beta_{djt}^\mathcal{C}  \log(P_{djt}^c)$  

--
4. Regress residual on 5-year average of climate variables, 

$$\hat{\delta}_{djt} = \beta^Z_{1} \text{Fire}_j + \beta^Z_{2} \text{Flood}_j + \beta^Z_{3} \text{Precip}_{jt} + \beta^Z_{4} \text{No Rain}_{jt} + \\ \sum_\tau \beta^Z_5(\tau) D_{\tau jt} + \omega_d + \omega_t + \xi_{djt}$$
...
$$\hat{\delta}_{djt} = \color{#EC7662}{\beta^Z_{1} \text{Fire}_j} + \color{#EC7662}{\beta^Z_{2} \text{Flood}_j} + \beta^Z_{3} \text{Precip}_{jt} + \beta^Z_{4} \text{No Rain}_{jt} + \\ \sum_\tau \beta^Z_5(\tau) D_{\tau jt} + \omega_d + \omega_t + \xi_{djt}$$
...
$$\hat{\delta}_{djt} = \beta^Z_{1} \text{Fire}_j + \beta^Z_{2} \text{Flood}_j + \color{#EC7662}{\beta^Z_{3} \text{Precip}_{jt}} + \color{#EC7662}{\beta^Z_{4} \text{No Rain}_{jt}} + \\ \sum_\tau \beta^Z_5(\tau) D_{\tau jt} + \omega_d + \omega_t + \xi_{djt}$$
...
$$\hat{\delta}_{djt} = \beta^Z_{1} \text{Fire}_j + \beta^Z_{2} \text{Flood}_j + \beta^Z_{3} \text{Precip}_{jt} + \beta^Z_{4} \text{No Rain}_{jt} + \\ \color{#EC7662}{\sum_\tau \beta^Z_5(\tau) D_{\tau jt}} + \omega_d + \omega_t + \xi_{djt}$$

- $\text{Fire}_j$ and $\text{Flood}_j$ are median fire and flood risk scores in city $j$  
- $\text{Precip}_{jt}$ and $\text{No Rain}_{jt}$ are total precip and Pr(<1mm of precip) in year $t$    
- $\color{#EC7662}{\beta^Z_5(\tau)}$ is a cubic B-spline over days at temperature $\tau$, $D_{\tau jt}$  
...
$$\hat{\delta}_{djt} = \beta^Z_{1} \text{Fire}_j + \beta^Z_{2} \text{Flood}_j + \beta^Z_{3} \text{Precip}_{jt} + \beta^Z_{4} \text{No Rain}_{jt} + \\ \sum_\tau \beta^Z_5(\tau) D_{\tau jt} + \color{#EC7662}{\omega_d} + \color{#EC7662}{\omega_t} + \xi_{djt}$$

- $\text{Fire}_j$ and $\text{Flood}_j$ are median fire and flood risk scores in city $j$  
- $\text{Precip}_{jt}$ and $\text{No Rain}_{jt}$ are total precip and Pr(<1mm of precip) in year $t$  
- $\beta^Z_5(\tau)$ is a cubic B-spline over days at temperature $\tau$, $D_{\tau jt}$  
...
```{r, results = 'asis'}
  levels_mod_cal = qread('data/levels_mod_cal.qs')
  etable(
    levels_mod_cal[rhs ='spline'],
    drop = 'spline',
    digits = "r3",
    digits.stats = "r2",
    style.tex = style.tex(
...
```{r results='asis'}
  cat_rank_dt = read.fst(
    "data/climate-amenities-rankings-2019-decomp.fst",
    as.data.table = TRUE
  )
  # Table with rankings by category
  kbl(
    cat_rank_dt[c(1:6,15:20)],
    "html",
    linesep = ''
  )|>
  #add_header_above(
  ...
  #)|>
  pack_rows("Best Cities", 1,6) |>
  pack_rows("Worst Cities", 7,12) 
...
```{r results='asis'}
  ...
  #) |>
  row_spec(0,bold=TRUE) |>
  row_spec(c(6,8), background = '#FFCC77')|>
  #column_spec(
  #  3, 
  #  color = "white",
  #  background = spec_color(cat_rank_dt[c(1:6,15:20)]$Hot, scale_from = c(1,70)),
...
- Can also multiply by wages to turn it into a dollar value  
...
With the model estimated, we run two types of counterfactuals, 

1. **Effect of Climate Change To-Date**: Plug in 1980's average climate to today's economy for effect of climate change thus far   
--
...
knitr::include_graphics("figures/model/general-eqm/cv-ind-1990-2019.jpeg")
```

- Welfare loss for median black HH over **2x larger** than median white HH  
- This amounts to damages of $750 vs $350 (or 1.7% vs 0.6% of income) 
...
Recall we had three proposed mechanisms 
1. **Greater exposure to climate change** based on *ex-ante* location  
...
```{r, results = 'asis'}
  decomp_dt =
    read.fst(
      "data/fix-loc-decomp-1990-2019.fst",
      as.data.table = TRUE
    )[,':='(
      decomp = fcase(
...
Black HHs worse off by **1.64%** of income on average with fixed locations and prices,    
--
...
Allowing households to sort **marginally reduces** the average black-white welfare gap, 
--
but the conditional welfare gap **grows to over 1%**.  
...
```{r out.width='100%', fig.align='center'}
knitr::include_graphics("figures/model/general-eqm/ts-ej-gap.jpeg")
```
...
- Black HHs have faced **2x larger** climate damages to-date  
...
[Vertical Migration Externalities](#vme) **RSUE, 2023**.   
[Optimal Subsidies for Residential Solar](#solar-spatial) *under review*.   
[Perinatal Health Effects of Herbicides](#gly) *working paper*.   
[Means Tested Solar Subsidies](#solar-income) *in progress*.   
[Valuing Water Quality: Benefits transfer with distance decay](#vme) *in progress*.
...
```{r vme, out.width = "60%"}
knitr::include_graphics('figures/other-papers/colas-saulnier-map.jpeg')
```


.smaller[State income taxes affect federal income tax revenue by shifting the spatial distribution of households between high- and low-productivity states, thereby changing household incomes and tax payments.
- We express these fiscal externalities in terms of estimable statistics 
- The externalities vary widely by state from positive to negative 
- Every dollar of California tax revenue raised decreases federal income tax revenue by 39 cents  ]
...
.pull-left[
```{r os1, echo = FALSE, out.width="100%", fig.cap = "Optimal solar subsidies"}
knitr::include_graphics("figures/other-papers/state-subsidy-opt-cost-neutral-welfare-max.jpeg")
```
]

.pull-right[
```{r os2, echo = FALSE, out.width="100%", fig.cap = "Installations as a % of optimal"}
knitr::include_graphics("figures/other-papers/install-ratio-cost-neutral-welfare-max.jpeg")
```
]  
...
.pull-left[
```{r ms1, echo = FALSE, out.width="100%"}
knitr::include_graphics("figures/other-papers/inc-bin-log-kwh-cap-rel-ols.jpeg")
```
]

.pull-right[
```{r ms2, echo = FALSE, out.width="100%"}
knitr::include_graphics("figures/other-papers/Opt_Sub_BincLatentInc1Sc1_Compare.png")
```
]  


.smallest[
  We study optimal design of income-contingent subsidies for residential solar panels. We document that the responsiveness of installation rates to subsidies is twice as high among low-income locations compared to high-income locations. Using these empirical elasticities, we estimate a model which embeds a solar panel installation decision into a dynamic consumption/savings framework with borrowing constraints. Counterfactual simulations reveal that switching to the production-maximizing income-contingent subsidies leads to a three-fold increase in public funds received by low-income households and a 4% increase in national solar production. Means-tested subsidies are justified on both equity and efficiency grounds.
]


---
name: gly
# Perinatal Health Effects of Glyphosate  

.pull-left[
```{r gly1, echo = FALSE, out.width="100%"}
knitr::include_graphics("figures/other-papers/fig-co-fs-1-1.svg")
```
]

.pull-right[
```{r gly2, echo = FALSE, out.width="100%"}
knitr::include_graphics("figures/other-papers/fig-rf-dbwt-1-1.svg")
```
]  

**Research Question** What are the health effects of the most widely used herbicide in the world?   
**Methods** Leverage the timing of the release of GM technology paired with spatial variation in the suitability for crops with GM varieties to estimate a causal effect of glyphosate on infant health.   
**Main Finding** Glyphosate causes a decrease in birthweight and gestation length. These effects come from local spraying rather than that from upstream. Effects are concentrated among most at risk births. 
...
Since we have cobb-douglas utility, $\mathcal{C}^*_{dj} = \frac{\alpha_{dj}^\mathcal{C}  W_{dj}}{\boldsymbol{\alpha_{dj}}P_{dj}^c}$

\begin{align}
    \small H^*_{dj} &= M^c_{dj} \left(P^E_{dj}\right)^{\frac{1}{1-\rho_c}}\times\frac{\alpha_d^\mathcal{C}  W_{dj}}{\tilde{\alpha}_{d}}\\
...
\begin{align*}
  ...
  &= \alpha_d^{l}+\frac{\kappa_E}{\rho_\mathcal{E}}\frac{P^E_j E_{dj}}{W_{dj}}
\end{align*}
...
wage_reg = qread('data/mod_wages_educ.qs')
...
```{r energy-eos, results = 'asis'}
  ...
  dem_cbsa_mods = qread('data/dem_group_cbsa/spline-deg-7/mod.qs')
  cbsa_mods = qread('data/cbsa/spline-deg-7/mod.qs')
  year_mods = qread('data/year/spline-deg-7/mod.qs')
  dem_mods = qread('data/dem_group/spline-deg-7/mod.qs')
  dem_year_mods = qread('data/dem_group_year/spline-deg-7/mod.qs')
...
.footnote[ [[return]](#climate-amenities-main) ]
...
change_decomp_dt = read.fst(
  "data/climate-amenity-change-1990-2019-decomp.fst",
  as.data.table =TRUE
) |>
setnames(old = 'Change (\\%)','Log Diff')

kbl(
  change_decomp_dt[c(1:6, 65:70)],
  "html",
  linesep = ''
)|>
#add_header_above(c(' ' = 2, "Category Change (\\%)" = 3)) |>
row_spec(0,bold=TRUE) |>
pack_rows("Improving Cities", 1,6) |>
pack_rows("Worsening Cities", 7,12)
```
...
$$\small \color{#EC7662}{\mathbb{E}[V_i(\boldsymbol{Z})]} = \bar{\gamma} + \log\left(\sum\limits_{j' \in J}\exp\left(\beta_{e}^w \log W_{ej'} + \beta_{dj'}^\mathcal{C} \log P_{d}^\mathcal{C}(Z_{j'}) + \beta^Z_d \cdot Z_{j'} + \xi_{dj'} + g(j', \boldsymbol{b}_i)\right)\right)$$

where $\bar{\gamma}$ is Euler's constant.
...
```{r ind-reg-results, results = 'asis'}
  ...
  ind_distr_mod = qread('data/ind-distr-mod.qs')
...
```{r results = 'asis'}
  tmp_dt = 
    read.fst(
      "data/change-in-population-1990-2019.fst",
      as.data.table = TRUE
    ) |>
...
pagedown::chrome_print(
  "saulnier-cal-poly.html",
  output = "saulnier-cal-poly.pdf"
)
```
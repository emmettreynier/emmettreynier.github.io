...
subtitle: "Emmett Reynier (joint with John Morehouse)"
author: |
  University of Oregon and ORISE at US EPA
date: |
  Heartland Workshop
output: 
...
- **Energy Justice** literature shows Black HH's spend more on energy relative to observably similar white HH's .smallest[[[Lyubich 2020]](https://evalyubich.com/files/race_energyGap.pdf)]  

<center>**Conditional Black-White Energy Expenditure Gap**</center> 
```{r out.width='80%', fig.align="center"}
knitr::include_graphics('figures/energy/energy-exp-gap-tot.jpeg')
```
...
1. Create a .hi-pink[quantitative spatial equilibrium model] of US labor markets  
   ...
   - Estimate with household data from the census/ACS and historical climate data from PRISM   
--

1. Simulate .hi-pink[counterfactual climate] scenarios  
  - To-date, climate damages are **22% larger** for black vs white HH's  
  - Also larger for **low-income households** 
...
1. Does a .hi-pink[$3 Billion IRA program] address unequal impacts?  
  - Add **place-based, means-tested subsidies** to the model  
  - Current program benefits low-income households generally, but not proportional to damages from climate change   
...
Households pick a .hi-pink[city to live in] based on    
...
.footnote[[[parameters]](#kappas-main) ]     


???
- Identification from deviations in the weather from a city's average  
- Simulate effect on comfort price, the unit cost function for comfort  
...
levels_mod_cal = qread(here(base_path,'levels_mod_cal.qs'))


data.table(coeftable(
  levels_mod_cal[rhs ='dd'][[1]], 
  keep = 'dd|ppt|median'
))[,
  sd := 1#c(
...
- Relies on cross-sectional variation, with controls for geographic characteristics of cities and demographics    
- Robust to alternative model specifications .smallest[[[details]](#climate-amenities-alt)]  

.footnote[[[raw regression]](#climate-amenities-main-pt2-regression) [[heterogeneity]](#climate-amenities-het)]
...
knitr::include_graphics(
  path = "figures/model/general-eqm/inc-q10-cv-perc-5y.jpeg"
)
```

.footnote[[[distribution]](#cv-race-distr)]


???

- Welfare loss for median Black HH **22% larger** than median white HH  
- This amounts to damages of 2.7% vs 2.2% of income  
...
```{r out.width='100%', fig.align='center'}
knitr::include_graphics(
  path = "figures/model/general-eqm/ts-ej-gap.jpeg"
)
```
...
We have three mechanisms driving inequality 
...
1. **Mechanical**: Fix locations, rent, and wages

--
1. **Sorting**: Allow migration, but wages rent are fixed    
...
$\implies$ We find **ex-ante distribution** and differences in **moving costs** to be driving welfare gaps. .smallest[[[table]](#decomp-tab)] 

---
class: inverse, middle
# IRA Inspired Subsidies  
...
.footnote[[[program details]](#ira-initial-details) [[model details]](#ira-model-details)]

The Inflation Reduction Act allocated the EPA .hi-pink[$3 Billion] to fund projects in **disadvantaged communities** with stated goals of  

- Increasing community climate resilience  
- Building community capacity to address environmental/climate justice    
- Reducing pollution  

--

We incorporate subsidies that are  
- **Place-based**: Distribute a portion of total funds to each city   
- **Means-tested**: Must be below an income threshold to be eligible  

--

Eligible households divide that city's total equally.  


---
# Policy counterfactuals  

We are interested in how to **distribute the subsidies across space**  

1. .hi-green[Disadvantaged Population]
  - Choose city share proportional .hi-green[number of disadvantaged households] currently living in each city  
  - Most similar to current policy  
--

2. .hi-pink[Negative Climate Change]   
  - Choose city share proportional to .hi-pink[losses in climate amenities]     
  - Helps places that are most hurt by climate change  
  - Could increase climate exposure if households are mobile  
--

1. .hi-purple[Positive Climate Change]   
  - Choose city share proportional to .hi-purple[gains in climate amenities]     
  - Incentivizes people to move away from climate exposed cities     
  - Pays many "inframarginal" households who are already relatively well off from climate perspective  



---
name: ira-sim-cv-diff
# Welfare effects of adding subsidies  

We order households according to their **CV from climate change to date**,  then we calculate **CV with subsidies** in 2019 relative to without. 
...
knitr::include_graphics(
  path = "figures/model/ira-sim/subsidy-hiloincome-flex-only.jpeg"
)
...
.footnote[[[levels]](#ira-sim-cv-levels) [[population]](#ira-sim-pop) [[wages and rents]](#ira-sim-wages-rent)]



???

For .hi-purple[Positive]/.hi-pink[Negative] Climate Change distributions 
- Low income HH lose **15-20%** of benefits from GE adjustments      
- Generates **1-2% change** in climate exposure  

TODO: To put in appendix: 
- EJ gap 
- CV relative to 1990  
- Ratio of fixed vs flexible  
- Population, rent, wage changes by city  


---
name: ira-sim-climate-diff
# Change in exposure for low-income  

```{r, out.width = "90%", fig.align='center'}

knitr::include_graphics(
  path = "figures/model/ira-sim/low-inc-fsf-temp-change.jpeg"
)

```


.footnote[[[amenities]](#ira-sim-exposure)]
...
- Black HHs have faced **22% larger** climate damages to-date  
- Gaps will continue to widen with further emissions
...
- Giving subsidies to **places** that have been damaged by climate change targets *people* with largest climate damages    
- But also shifts low-income population into hotter, more flood prone areas  
...
$$\color{#4682B4}{\mathcal{C}(H,\mathcal{E}|d,j)} = \left(H^{\rho_c} + \color{#9370DB}{\mathcal{E}}^{\rho_c} \right)^{1/\rho_c}$$
...
$$\color{#4682B4}{P^\mathcal{C}_{dj}} = \left( {P^H_{j}}^{1-\sigma_c}+ {\color{#9370DB}{P^\mathcal{E}_{dj}}}^{1-\sigma_c}\right)^{\frac{1}{1-\sigma_c}}$$
...
.footnote[[[return]](#comfort-price-het)]
...
```{r, results = 'asis'}
  decomp_dt =
    read.fst(
      ...
      decomp = fcase(
        decomp == 'mech', 'Fixed Location, no comfort adjustment',
        decomp == 'sorting', 'Sorting, fixed prices',
        decomp == 'general-eqm', 'Fully Flexible',
        decomp == 'mech-ec', 'Fixed Location'
      ) |> factor(levels = c('Fixed Location, no comfort adjustment','Fixed Location','Sorting, fixed prices','Fully Flexible'))
    )]
  ...
  base_decomp_kbl = 
    dcast(
      decomp_dt[
        ...
        decomp != 'Fixed Location, no comfort adjustment' & 
        effect_from == 'both'
      ],
...
Black HHs worse off by **0.81%** of income on average with fixed locations and prices,    
--
...
---
name: ira-subsidies-details
# EPA Community Change Grants  

Biden Administration's [Justice40 initiative](https://www.whitehouse.gov/environmentaljustice/justice40/): 40% of benefits from environmental rulemaking goes towards disadvantaged communities   

Initial round of .hi-pink[$325 million] in funding for **17 projects** take on   
- Energy retrofits (x10) 
- Community resilience hubs and public transit (x10)  
- Expand green space and improve parks (x9)  
- Solar installations and storage (x3)  

Applications for rest of funds are due in November, 2024.  



.footnote[[[return]](#ira-subsidies) [[full list]](#ira-initial-round-full)]
    

---
# Disadvantaged communities 

A **disadvantaged community** is a census tract   

> historically marginalized and overburdened by pollution and underinvestment in housing, transportation, water and wastewater infrastructure, and health care  

```{r echo = FALSE, cache = TRUE, fig.width=12, fig.height=8.57, out.width='65%', message = FALSE, warning = FALSE, output = FALSE}
pacman::p_load(sf, tigris, collapse)
...
tract_sf = 
  ...
  st_transform(crs = 9311) 
raw_ej_dt = fread(here('data/raw/other/epa-disadvantaged-cbg.csv'))[,.(
  GEOID = str_pad(ID, side ='left', width = 12, pad = '0') |> str_sub(1,11), 
  GEOID_cbg = str_pad(ID, side ='left', width = 12, pad = '0'), 
  disadvantaged = Disadvantaged == 'Yes'
)] |> gby(GEOID) |> fmode()

inner_join(
  tract_sf, 
  raw_ej_dt[disadvantaged == TRUE], 
  by = 'GEOID'
) |>
ggplot() +
geom_sf(color = '#708090', fill = '#708090') + 
geom_sf(data = state_sf, color = 'black', fill = NA, linewidth = 0.5) + 
theme_void()
...
.footnote[from [CEJST](https://screeningtool.geoplatform.gov/en/) and [EJSCREEN](https://www.epa.gov/environmentaljustice/inflation-reduction-act-disadvantaged-communities-map) [[return]](#ira-subsidies)]



---
name: ira-model-details
# Integrating subsidies into the model  

We model these subsidies as .hi-pink[place-based, means-tested] subsidies. We allow the government to pick three aspects of the program,  

1. $G$ is total amount of subsidies  
2. $w_j$ is the allocation of total to city $j$ such that $\sum_j w_j = 1$  
3. $I_{thresh}$ is the income threshold for eligibility  

--

Each city's total subsidy allocation, $G \times w_j$, is divided equally between households below the income threshold who choose to live there. 

$$\text{IRA}_{dj} = \frac{G \times w_j}{N^{disad}_{j}}\mathbb{I}\left(I_{dj} < I_{thresh}\right)$$


--

Government balances budget with a flat tax, $\tau$, on income 

$$G = \int_i \tau I_{dj} di$$


.smallest[[[return]](#ira-subsidies)]
...
knitr::include_graphics(
  path = "figures/model/ira-sim/subsidy-hiloincome-distr.jpeg"
)
...
First, we order households by **baseline CV** from 2019 - 1990 climate  

.footnote[[[return]](#ira-sim-cv-diff)]

---
name: ira-sim-cv-levels 
# CV from subsidies in levels  

```{r out.width='100%', fig.align='center'}
knitr::include_graphics(
  path = "figures/model/ira-sim/subsidy-hiloincome-levels.jpeg"
)
```
...
knitr::include_graphics(
  path = "figures/model/ira-sim/subsidy-hiloincome-flex.jpeg"
)
...
---
# Welfare effects by program size  

```{r, out.width = "75%", fig.align='center'}

knitr::include_graphics(
  path = "figures/model/ira-sim/subsidy-hiloincome-flex-magnitudes.jpeg"
)
...
---
name: ira-sim-pop
# Population changes from subsidies   



```{r out.width='100%', fig.align='center'}
knitr::include_graphics(
  path = "figures/model/ira-sim/cbsa-pop-change-diff-magnitudes.jpeg"
)
```
...
---
name: ira-sim-rent
# Rent changes from subsidies   


```{r out.width='100%', fig.align='center'}
knitr::include_graphics(
  path = "figures/model/ira-sim/cbsa-rent-change-diff-magnitudes.jpeg"
)
```
...
---
name: ira-sim-wages
# Wage changes from subsidies   


```{r out.width='100%', fig.align='center'}
knitr::include_graphics(
  path = "figures/model/ira-sim/cbsa-wage-change-diff-magnitudes.jpeg"
)
```
...
---
name: ira-sim-exposure
# Expsoure changes from subsidies   


```{r out.width='100%', fig.align='center'}
knitr::include_graphics(
  path = "figures/model/ira-sim/exposure-hiloincome-flex.jpeg"
)
```
